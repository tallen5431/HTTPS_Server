...
/**
 * Auto-Discovery Script for HTTPS Server Manager
 *
 ...
 *   node discover-projects.js /home/jupyter-tj/projects --output config.json
...
/**
 * Generate URL path from project name
 */
function generateUrlPath(projectName) {
  return '/' + projectName
    .toLowerCase()
    .replace(/\s+/g, '-')
    .replace(/[^a-z0-9-]/g, '')
...
function discoverProjects(projectsDir) {
  ...
  for (const entry of entries) {
    ...
    // Add URL if PORT is detected (enables Caddy integration)
    if (env.PORT) {
      const urlPath = generateUrlPath(entry.name);
      program.url = urlPath;

      // Add URL_PREFIX for frameworks that need it
      if (hasFlask || hasStreamlit) {
        env.URL_PREFIX = urlPath;
      }
    }
    ...
    if (env.PORT) {
      comments.push('Caddy integration enabled');
    }
 ...
 */
function main() {
  console.log('🔍 HTTPS Server Manager - Project Discovery\n');
  ...
  console.log(`  Projects with Caddy integration: ${projects.filter(p => p.url).length}`);
  ...
  if (DRY_RUN) {
    ...
    console.log('  2. Start the manager: node server.js');
